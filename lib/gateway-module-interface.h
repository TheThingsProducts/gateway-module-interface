...
typedef enum
{
    ...
    GATEWAY_MODULE_CMD_BOOTLOADER_MODE	= 0x50, // Host -> Module Invalidates the checksum of the application. Need to followed by a RESET command to switch to the Bootloader.
...
void GatewayModuleInterface_init(gateway_module_interface_write_lock_t write_lock, gateway_module_interface_write_t write, gateway_module_signal_wait_t signal_wait, gateway_module_signal_set_t signal_set, gateway_module_receive_callback_t receive_callback, gateway_module_log_t log);
